...
import { useGetSurveyByIdQuery, useUpdateSurveyMutation } from "@/store/services/api"
import {
  setSurveyName,
  setExpireDate,
  setParticipants,
...
export default function EditSurvey() {
  ...
  const { data: survey, isLoading, error } = useGetSurveyByIdQuery(surveyId)
  ...
  const handleUpdateSurvey = async () => {
    try {
      await updateSurvey({
        ...
        name: surveyName,
        expireDate,
        questions: questions.map(({ id, ...q }) => q), // remove ids for new questions
        participants
      }).unwrap()
      ...
      alert("Anket güncellenirken hata oluştu: " + (err.data?.message || err.message))
    }
  }
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-destructive">Anket yüklenirken hata oluştu: {error.message}</div>
      </div>
    )
  }

  // not draft - cannot edit
  if (survey?.status !== "DRAFT") {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-xl font-semibold text-gray-800 mb-2">Anket Düzenlenemiyor</h2>
          <p className="text-muted-foreground mb-4">
            Sadece TASLAK durumundaki anketler düzenlenebilir.
          </p>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <Badge variant="draft">{survey?.status}</Badge>
        </div>
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-4">
            ...
            <Button
              ...
              disabled={isUpdating}
            >
              {isUpdating ? "Güncelleniyor..." : "Anketi Güncelle"}
            </Button>
